...
<el-breadcrumb separator-class="el-icon-arrow-right" class="breadnaviUser">
  ...
  <el-table :data="commentslist" border="" class="userlisttable" stripe=""> 
      ...
      <el-table-column label="数据ID" prop="messageid"></el-table-column>
      <el-table-column label="留言用户" prop="userid"></el-table-column>
      <el-table-column label="联系方式" prop="contact"></el-table-column>
      <el-table-column label="留言内容" prop="context"></el-table-column>
...
<script>
import {getMessageListByPage, permitArticle} from '../../api'
...
export default {
    ...
    methods: {
        ...
        // 获取文章列表
        getArticleListAPI(){
            getMessageListByPage(this.queryInfo.pagenum, this.queryInfo.pagesize, this.queryInfo.query).then(res => {
                if(res.data.code !==1) return this.$message.error(res.data.msg)
